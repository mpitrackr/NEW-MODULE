...
class StockBarcodeLot(models.TransientModel):
    ...
    inventory_barcode_lot_line_ids = fields.One2many('inventory_barcode.lot.line', 'inventory_barcode_lot_id')
    ...
    def default_get(self, fields):
        ...
        qty_reserved = 0.0
        qty_done = 0.0
        if 'inventory_barcode_lot_line_ids' in fields and self.env.context.get('candidates'):
            ...
            for ml in candidates:
                ...
                qty_reserved += ml.product_uom_qty
                qty_done += ml.qty_done
            res['inventory_barcode_lot_line_ids'] = [(0, 0, x) for x in lines]
    ...
    def _update_quantity_done(self):
        self.qty_done = sum(self.inventory_barcode_lot_line_ids.mapped('qty_done'))

    def on_barcode_scanned(self, barcode):
        suitable_line = self.inventory_barcode_lot_line_ids.filtered(lambda l: l.lot_name == barcode or not l.lot_name)
        vals = {}
        if suitable_line:
            ...
            vals['lot_name'] = barcode
            vals['qty_done'] = 1
            vals['inventory_barcode_lot_id'] = self.id
            self.env['inventory_barcode.lot.line'].new(vals)
        self.update({'qty_done': self.qty_done + 1})
        return

    def validate_lot(self):
        for line in self.inventory_barcode_lot_line_ids:
            if line.lot_name:
                vals = {}
...
class StockBarcodeLotLine(models.TransientModel):
    _name = "inventory_barcode.lot.line"
    ...
    inventory_barcode_lot_id = fields.Many2one('inventory_barcode.lot')
    ...
    def onchange_qty_done(self):
        if self.inventory_barcode_lot_id.product_id.tracking == 'serial' and self.qty_done > 1:
            raise UserError(_('You cannot scan two times the same serial number'))
        self.inventory_barcode_lot_id._update_quantity_done()
