...
class InventoryBarcodeController(http.Controller):

    @http.route('/inventory_barcode/scan_from_main_menu', type='json', auth='user')
    ...
    def get_action(self, picking_id):
        ...
        """
        use_form_handler = request.env['ir.config_parameter'].sudo().get_param('inventory_barcode.use_form_handler')
        if use_form_handler:
            ...
            action = request.env.ref('inventory_barcode.inventory_barcode_picking_client_action').read()[0]
            params = {
                'model': 'stock.picking',
    ...
    @http.route('/inventory_barcode/rid_of_message_demo_barcodes', type='json', auth='user')
    def rid_of_message_demo_barcodes(self, **kw):
        ...
        action = request.env.ref('inventory_barcode.inventory_barcode_action_main_menu')
        action and action.sudo().write({'params': {'message_demo_barcodes': False}})

    @http.route('/inventory_barcode/get_set_barcode_view_state', type='json', auth='user')
