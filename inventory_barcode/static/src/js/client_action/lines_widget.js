odoo.define('inventory_barcode.LinesWidget', function (require) {
'use strict';
...
var LinesWidget = Widget.extend({
    template: 'inventory_barcode_lines_widget',
    events: {
    ...
    addProduct: function (lineDescription, model, doNotClearLineHighlight) {
        ...
        var $line = $(QWeb.render('inventory_barcode_lines_template', {
    ...
    */
     ...
     */
     _renderLines: function () {
         if (this.mode === 'done') {
             ...
             this._toggleScanMessage('picking_already_cancelled');X000TWJ0CB
        ...
        var $pageSummary = $(QWeb.render('inventory_barcode_summary_template', {
        ...
        if (this.page.lines.length) {
            var $lines = $(QWeb.render('inventory_barcode_lines_template', {
    ...
    _onClickDeleteLine: function (ev) {
       ev.preventDefault();
        ev.stopPropagation();
        var id = $(ev.target).parents('.o_barcode_line').data('id');
        this.trigger_up('delete_line', {id: id});
    },
