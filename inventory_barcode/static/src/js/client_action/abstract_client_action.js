odoo.define('inventory_barcode.ClientAction', function(require) {
  'use strict';
  ...
  var ViewsWidget = require('inventory_barcode.ViewsWidget');
  var HeaderWidget = require('inventory_barcode.HeaderWidget');
  var LinesWidget = require('inventory_barcode.LinesWidget');
  var SettingsWidget = require('inventory_barcode.SettingsWidget');
  ...
  var ClientAction = AbstractAction.extend({
    ...
    _getState: function(recordId, state) {
      ...
      if (state) {
        ...
        def = this._rpc({
          'route': '/inventory_barcode/get_set_barcode_view_state',
          'params': {
            'record_id': recordId,
            'mode': 'read',
    ...
    _loadNomenclature: function() {
      ...
      this.barcodeParser = new BarcodeParser({
        'nomenclature_id': this.currentState.nomenclature_id
      });
    ...
    _makePages: function() {
      ...
      if (this._getLines(this.currentState).length) {
        ...
        var groups = _.groupBy(this._getLines(this.currentState), function(line) {
          return _.map(self._getPageFields({
            line: true
          }), function(field) {
    ...
    _step_product: function(barcode, linesActions) {
      ...
      var product_barcode = ''
      if(barcode){
    	  product_barcode = barcode.split('|')[0]
      }
      if (product_barcode){
    	  var product = this._isProduct(product_barcode);  
      }
      else{
      ...
      if (product) {
        ...
        if (res.isNewLine) {
          if (this.actionParams.model === 'stock.inventory') {
            ...
            if (this.actionParams.model === 'stock.picking') {
              var line_id = res.virtualId;
              return this._rpc({
                model: 'stock.picking',
                method: 'set_value_from_barcode',
                args: [line_id, barcode, res.isNewLine],
              }).then(function(data) {
                res.lineDescription.lot_production_date = data.lot_production_date;
                res.lineDescription.lot_expiry_date = data.lot_expiry_date;
                res.lineDescription.packing_size = data.packing_size;
                res.lineDescription.lot_name = data.lot_name;
                res.lineDescription.lot_id = data.lot_id;
                linesActions.push([self.linesWidget.addProduct, [res.lineDescription, self.actionParams.model]]);
                return Promise.resolve({
                  linesActions: linesActions
                });
        ...
        var fail = function(specializedErrorMessage) {
          ...
          if (!self.scannedLines.length) {
            if (self.groups.group_tracking_lot) {
                errorMessage = _t("You are expected to scan one or more products or a package available at the picking's location. This is from Var Fail. Scanned Lines length = "+self.scannedLines.length + " Barcode = "+barcode);
            } 
            else {
                errorMessage = _t('You are expected to scan one or more products. abstarct_client_action.js file , fail called line 953. ' + barcode);
            }
            return Promise.reject(errorMessage);
          }
    ...
    _step_package: function(barcode, linesActions) {
      ...
      var search_read_quants = function() {
        ...
        return self._rpc({
          ...
          domain: [
            ['name', '=', barcode],
            ['location_id', 'child_of', self.scanned_location.id]
          ],
          limit: 1,
        });
      };
    ...
    _step_destination: function(barcode, linesActions) {
      ...
      if (!isChildOf(this.currentState.location_dest_id, destinationLocation)) {
        ...
        if (!this.scannedLines.length || this.mode === 'no_multi_locations') {
          if (this.groups.group_tracking_lot) {
            errorMessage = _t("You are expected to scan one or more products or a package available at the picking's location. This is error from _step_destination. Scanned Line Length = "+this.scannedLines.length);
          } else {
            errorMessage = _t('You are expected to scan one or more products.  abstarct_client_action.js file , destination location 1300 ' + barcode);
          }
          return Promise.reject(errorMessage);
        }
        var self = this;
    ...
    _onMainMenu: function() {
      ...
      self._save().then(function() {
        self.do_action('inventory_barcode.inventory_barcode_action_main_menu', {
  ...
  core.action_registry.add('inventory_barcode_client_action', ClientAction);
