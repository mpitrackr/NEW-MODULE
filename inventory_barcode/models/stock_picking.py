...
class StockPicking(models.Model):
    ...
    def get_po_to_split_from_barcode(self, barcode):
        ...
        view_id = self.env.ref('inventory_barcode.view_barcode_lot_form').id
        return {
            'name': _('Lot/Serial Number Details'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'inventory_barcode.lot',
            'views': [(view_id, 'form')],
            'view_id': view_id,
            'target': 'new',
            'context': action_ctx}
    ...
    def open_picking_client_action(self):
        ...
        """
        self.ensure_one()
        use_form_handler = self.env['ir.config_parameter'].sudo().get_param('inventory_barcode.use_form_handler')
        if use_form_handler:
            view_id = self.env.ref('stock.view_picking_form').id
            return {
                'name': _('Open picking form'),
            ...
            action = self.env.ref('inventory_barcode.inventory_barcode_picking_client_action').read()[0]
            params = {
                'model': 'stock.picking',
                'picking_id': self.id,
    ...
    @api.model
    def set_value_from_barcode(self,move_line_id,barcode,isNewLine):
        ...
        if barcode:
            ...
            line_id = False
            if not isNewLine:
                line_id = self.env['stock.move.line'].search([('id','=',move_line_id)],limit=1)
            months = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"]    
            if lot_production_date:
                line_date = lot_production_date.split("-")
                month = line_date[1]
                month1 = month.lower()
                month = months.index(month1) + 1
                if month <= 9:
                    month = "0"+str(month)
                lot_production_date = str(line_date[2])+"-"+ str(month)+"-"+ str(line_date[0])

            if lot_expire_date:
                line_date = lot_expire_date.split("-")
                month = line_date[1]
                month1 = month.lower()
                month = months.index(month1) + 1
                if month <= 9:
                    month = "0"+str(month)
                lot_expire_date = str(line_date[2])+"-"+ str(month)+"-"+ str(line_date[0])

            if line_id:
                line_id.write({'lot_expiry_date':lot_expire_date,
...
class StockPickingType(models.Model):
    ...
    def get_action_picking_tree_ready_kanban(self):
        return self._get_action('inventory_barcode.stock_picking_action_kanban')
